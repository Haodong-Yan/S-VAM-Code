...
class HulcWrapper(gym.Wrapper):
    ...
    def step(
        ...
        try:
            ...
            raise TypeError(
                f"Expected env.step to return an iterable, got {type(step_result)}"
            ) from None
        ...
        if result_len == 4:
            ...
            o, r, terminated, truncated, i = step_tuple
            d = terminated or truncated
        else:
            raise ValueError(
                f"Expected 4 or 5 values from env.step, got {result_len}: {type(step_result)}"
            )

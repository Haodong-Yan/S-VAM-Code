...
class RolloutVideo:
    ...
    def _log_videos_to_file(self, global_step, save_as_video=False):
        ...
        for video, tag in zip(self.videos, self.tags):
            ...
            self._write_video_tensor(video, filename, save_as_video=save_as_video)
    ...
    def save_last_subtask_video_to_file(self, tag_suffix, current_step, save_as_video=False, subfolder=None):
        if not self.videos:
            return
        video = self.videos[-1][:, self.sub_task_beginning : self.step_counter]
        if video.shape[1] == 0:
            return
        tag = f"{self.tags[-1]}__{tag_suffix}".replace("/", "_")
        out_dir = self.save_dir
        if subfolder:
            out_dir = self.save_dir / subfolder
            os.makedirs(out_dir, exist_ok=True)
        if save_as_video:
            filename = str(out_dir / f"{tag}_{current_step}.mp4")
        else:
            filename = out_dir / f"{tag}_{current_step}.gif"
        self._write_video_tensor(video, filename, save_as_video=save_as_video)

    def _write_video_tensor(self, video, filename, save_as_video=False):
        ...
        if save_as_video:
            ...
            clip.write_videofile(filename, codec='libx264', bitrate="5000k")
        else:

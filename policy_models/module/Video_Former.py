...
class PerceiverAttentionLayer(nn.Module):
    ...
    def forward(self, features, latents, return_attn_map: bool = False):
        ...
        out = self.to_out(out)
        if return_attn_map:
            # Keep only latent-query -> perceptual-feature attention (exclude latent tokens in kv_input).
            attn_map = alphas[..., :n_features]
            return out, attn_map
        return out
...
class Video_Former_3D(nn.Module):
    ...
    def __init__(
        ...
        # Allow callers to enable attention tracking without changing every callsite.
        self.track_attention_default = False
    ...
    def forward(
        ...
        track_attention = bool(track_attention or self.track_attention_default)
        # Apply attention and feed forward layer
        layer_attn_maps = [] if track_attention else None
        if self.use_temporal:
            for attn, Temp_attn, ffw in self.layers:
                if track_attention:
                    attn_out, attn_map = attn(x_f, x, return_attn_map=True)
                    layer_attn_maps.append(attn_map)
                else:
                    attn_out = attn(x_f, x)
                x = x + attn_out
        ...
        norm = self.norm(x)
        if track_attention and layer_attn_maps:
            # (layers, B*T, heads, q, n_features) -> (layers, B, T, heads, q, n_features)
            attn_maps = torch.stack(layer_attn_maps, dim=0)
            attn_maps = attn_maps.view(attn_maps.shape[0], batch_size, max_length, *attn_maps.shape[2:])
            self.last_attention_map = attn_maps
        else:
            self.last_attention_map = None
...
class Video_Former_2D(nn.Module):
    ...
    def forward(self, x_f: torch.Tensor, mask: torch.BoolTensor = None, track_attention: bool = False):
        """Run perceiver resampler on the input visual embeddings

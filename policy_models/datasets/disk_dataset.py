...
class ExtendedDiskDataset(DiskDataset):
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.enable_future_rgb = enable_future_rgb
    ...
    def _load_episode(self, idx: int, window_size: int) -> Dict[str, np.ndarray]:
        ...
        """
        start_idx = self.episode_lookup[idx]
        end_idx = start_idx + self.action_seq_len + self.obs_seq_len
        rgb_obs_cfg = list(self.observation_space["rgb_obs"]) if "rgb_obs" in self.observation_space else []
        base_rgb_keys = {str(k) for k in rgb_obs_cfg if not str(k).endswith("_future")}
        if self.enable_future_rgb:
            future_rgb_keys = {str(k) for k in rgb_obs_cfg if str(k).endswith("_future")}
        else:
            future_rgb_keys = set()
        future_key_map = {
            base_key: f"{base_key}_future"
            for base_key in base_rgb_keys
            if f"{base_key}_future" in future_rgb_keys
        }
        keys = list(chain(*self.observation_space.values()))
        keys.remove("language")
        keys.append("scene_obs")
        ...
        for key in keys:
            ...
            if 'gen' in key:
                continue
            if key_str in future_rgb_keys:
                continue
            stacked_data = np.stack([ep[key] for ep in episodes])
            if key_str in base_rgb_keys:
                episode[key_str] = stacked_data[: self.obs_seq_len]
                if self.enable_future_rgb:
                    future_key = future_key_map.get(key_str)
                    if future_key:
                        episode[future_key] = stacked_data[
                            self.obs_seq_len : self.obs_seq_len + self.action_seq_len
                        ]
            elif key_str == "rel_actions" or key_str == 'actions':
                episode[key_str] = stacked_data[

...
from model import HiddenToDA3Model, HiddenToDA3ModelWithRef
...
def train_one_epoch(
    ...
    for frames, hidden_states in progress:
        ...
        with torch.no_grad():
            targets = extract_da3_targets(frames, backbone)
            num_stages = _get_num_stages(model)
            targets = targets[:num_stages]
            ref_tokens_from_first_frame = extract_da3_targets(first_frame, backbone)
            ref_tokens_from_first_frame = ref_tokens_from_first_frame[0][:, :1]
        ...
        with torch.cuda.amp.autocast(enabled=scaler.is_enabled()):
            ...
            loss = _compute_stage_loss(preds_tokens, targets)
...
def main() -> None:
    ...
    token_dim = 1536  # DinoV2 vitb with cat_token=True
    ...
    backbone = load_da3_backbone(device, ckpt_path=args.backbone_ckpt, model_dir=args.model_dir)
